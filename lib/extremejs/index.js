...
exports.xget = function(url, callback, context) {
  ...
  res.writeHead = function(code) { 
    console.log('xget.code %d', code);
    this.code = code; 
  };
  res.end = function(str) {
    console.log('xget.entity %s', str);
    if(str)
  ...
  if(!context) context = {};
  processMethod(req, res, null, context);
}
...
function responseAfter(req, res, context, code, entity, input) {
  async.series([
    function(callback) {
      if(entity && code < 400) {
        var name = urlentity(req.url);
        var config = embeds[name];
        embed(req, context, entity, config, callback);
      }
      else
        callback();
    },
